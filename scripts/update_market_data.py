...
def fetch_ppi_yoy_with_fallback():
    # 중요: ppi_yoy는 시장에서 말하는 headline PPI, 즉 Final Demand PPI YoY로 맞춘다.
    # 이전 버전은 PPIACO(All Commodities)를 먼저 사용해 에너지/원자재 급등 시
    # BLS headline final demand PPI보다 과도하게 큰 값이 표시될 수 있었다.
    # 따라서 PPIACO는 제거하고 WPUFD4/PPIFIS Final Demand 계열만 사용한다.
    providers = [
        ("FRED WPUFD4 Final Demand PPI YoY", lambda: fetch_fred_monthly_yoy("WPUFD4", years_back=8)),
        ("BLS WPUFD4 Final Demand PPI YoY", lambda: fetch_bls_monthly_yoy(BLS_PPI_FINAL_DEMAND_SERIES_ID, "BLS PPI Final Demand", years_back=8, is_proxy=False)),
        ("FRED PPIFIS Final Demand PPI YoY fallback", lambda: fetch_fred_monthly_yoy("PPIFIS", years_back=8)),
    ]
...
def update_cpi_ppi_pass_through(data):
    ...
    item.update({
        "source": "Derived from CPIAUCSL-YoY and Final Demand PPI YoY",
        "sourceSeries": "CPIAUCSL-YoY-minus-WPUFD4-YoY",
